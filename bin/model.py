...
def intialCotrol(intialCotrol):
    ...
    cv2.resizeWindow("Control", 360, 240)
...
def perspective_warp(img,
    ...
    #Đối với các điểm đến tùy ý chọn một số điểm phù hợp để hiển thị lại kết quả bị khi bị lệch
    #không chính xác, nhưng phải đủ gần để xác định tâm
...
def sliding_window(img, nwindows=15, margin=50, minpix=1, draw_windows=True):
    ...
    if leftx.size and rightx.size:
        # Ghép một đa thức bậc hai cho mỗi
...
def get_curve(img, leftx, rightx):
    ...
    # Điều chỉnh đa thức mới cho x, y trong không gian thế giới
    left_fit_cr = np.polyfit(ploty * ym_per_pix, leftx * xm_per_pix, 2)
    right_fit_cr = np.polyfit(ploty * ym_per_pix, rightx * xm_per_pix, 2)
    # Tính bán kính mới của độ cong
